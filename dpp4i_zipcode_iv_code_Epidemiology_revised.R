...
# Load shapefiles ####
zcta_shp <- sf::read_sf("/Volumes/PACS$/Cordes, Jack/Zip Code Data/nhgis0046_shape/nhgis0046_shapefile_tl2017_us_zcta_2017", layer = "US_zcta_2017")
...
state_shp <- sf::read_sf("/Volumes/PACS$/Cordes, Jack/Zip Code Data/nhgis0047_shape/nhgis0047_shapefile_tl2017_us_state_2017", layer = "US_state_2017")
...
# Create folder for bias component plots
dir.create("/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots")

# Create folder for complier table ones
dir.create("/Volumes/PACS$/Cordes, Jack/TableOne_Compliers")

# Bias component plot graph function
graphfun_singleIV <- function(dat,location,graphname,lb=-0.5,ub=0.5) {
  
  col1 <- dat[,1]
  col2 <- dat[,2]
  col3 <- as.numeric(unlist(dat[,3]))
  col4 <- as.numeric(unlist(dat[,4]))
...
# Add complier variable
tecos_phys$ZIP_HIST_0_100_COMPLIER <- ifelse(tecos_phys$ZIP_HIST_0_100 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_zip_hist_0_100_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(ZIP_HIST_0_100) == FALSE),
                                                       factorVars = tecos_factor_vars_ps,
                                                       strata = "ZIP_HIST_0_100_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_zip_hist_0_100_compliers_export <- print(tecos_tableone_all_iv_zip_hist_0_100_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_zip_hist_0_100_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_zip_hist_0_100_compliers.csv")

# Distribution of outcomes by zipcode exposure and by actual exposure
tecos_iv_zip_hist_0_100_table <- filter(tecos_phys, is.na(ZIP_HIST_0_100) == F)
...
# Extract subset of covariates
tecos_phys_subset_0_100 <- tecos_phys %>% select(PID,
                                                 EXPOSURE,
                                                 ZIP_HIST_0_100,
                                                 AGE_CAT,
                                                 SEX,
...
# Turn race and region into dummy variables and join back to full dataset
tecos_phys_subset_0_100_race <- tecos_phys_subset_0_100 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
tecos_phys_subset_0_100 <- left_join(tecos_phys_subset_0_100, tecos_phys_subset_0_100_race, by = "PID")

tecos_phys_subset_0_100_region <- tecos_phys_subset_0_100 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
tecos_phys_subset_0_100 <- left_join(tecos_phys_subset_0_100, tecos_phys_subset_0_100_region, by = "PID")

# Remove original IV, PID, race, and region variables
tecos_phys_subset_0_100_exposure <- tecos_phys_subset_0_100 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_0_100)

# Obtain mean value of each variable by exposure status
tecos_phys_0_100_exposure_mean <- tecos_phys_subset_0_100_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                     SEX,
                                                                                                     OBESITY,
                                                                                                     OVERWEIGHT,
                                                                                                     SMOKE,
...
tecos_phys_0_100_exposure_mean <- tecos_phys_0_100_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
                                                                                "Alcohol abuse or dependence" = ALCOHOL,
                                                                                ...
                                                                                Anxiety = ANXIETY,
                                                                                "Frailty score" = FRAILTY_EMPIRICAL_V3,
                                                                                "Number antidiabetics" = N_ANTIDIABETICS,
                                                                                "Number diagnoses" = N_DIAGNOSES,
                                                                                "Number prescription drugs" = N_DRUG_RX,
                                                                                White = RACE_0,
                                                                                Black = RACE_1,
                                                                                Asian = RACE_2,
                                                                                "Native American" = RACE_3,
                                                                                Hispanic = RACE_4,
                                                                                "Other Race" = RACE_5,
                                                                                Northeast = REGION_0,
                                                                                South = REGION_1,
...
tecos_phys_0_100_exposure_mean <- melt(setDT(tecos_phys_0_100_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
tecos_phys_0_100_exp_mean <- tecos_phys_0_100_exposure_mean %>% filter(EXPOSURE == 1)
tecos_phys_0_100_unexp_mean <- tecos_phys_0_100_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
tecos_phys_0_100_exp_mean <- left_join(tecos_phys_0_100_exp_mean, tecos_phys_0_100_unexp_mean, by = "covariate")

# Calculate difference in means
tecos_phys_0_100_exp_mean$difference <- (tecos_phys_0_100_exp_mean$MEAN.x - tecos_phys_0_100_exp_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_0_100_exp_mean <- tecos_phys_0_100_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_0_100_exp_mean$contrast <- "By Sitagliptin"
tecos_phys_0_100_exp_mean$scale <- 1

# Reorder the variables
tecos_phys_0_100_exp_mean <- tecos_phys_0_100_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
tecos_phys_subset_0_100_iv <- tecos_phys_subset_0_100 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
tecos_phys_0_100_iv_mean <- tecos_phys_subset_0_100_iv %>% group_by(ZIP_HIST_0_100) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
tecos_phys_0_100_iv_mean <- tecos_phys_0_100_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_0_100_iv_mean <- melt(setDT(tecos_phys_0_100_iv_mean), id.vars = c("ZIP_HIST_0_100"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_0_100_iv1_mean <- tecos_phys_0_100_iv_mean %>% filter(ZIP_HIST_0_100 == 1)
tecos_phys_0_100_iv0_mean <- tecos_phys_0_100_iv_mean %>% filter(ZIP_HIST_0_100 == 0)

# Join means together
tecos_phys_0_100_iv_mean <- left_join(tecos_phys_0_100_iv1_mean, tecos_phys_0_100_iv0_mean, by = "covariate")

# Calculate difference in means
tecos_phys_0_100_iv_mean$difference <- (tecos_phys_0_100_iv_mean$MEAN.x - tecos_phys_0_100_iv_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_0_100_iv_mean <- tecos_phys_0_100_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_0_100_iv_mean$contrast <- "By Area 0-100 Candidate Instrument"
tecos_phys_0_100_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(ZIP_HIST_0_100) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
tecos_phys_0_100_iv_mean <- tecos_phys_0_100_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
tecos_phys_0_100_bias <- rbind(tecos_phys_0_100_exp_mean,tecos_phys_0_100_iv_mean)

# Bias component plot
graphfun_singleIV(tecos_phys_0_100_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_ZIP_HIST_0_100_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$ZIP_HIST_10_90_COMPLIER <- ifelse(tecos_phys$ZIP_HIST_10_90 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_zip_hist_10_90_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(ZIP_HIST_10_90) == FALSE),
                                                                 factorVars = tecos_factor_vars_ps,
                                                                 strata = "ZIP_HIST_10_90_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_zip_hist_10_90_compliers_export <- print(tecos_tableone_all_iv_zip_hist_10_90_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_zip_hist_10_90_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_zip_hist_10_90_compliers.csv")

# Distribution of outcomes by ZCTA exposure and by actual exposure
tecos_iv_zip_hist_10_90_table <- filter(tecos_phys, is.na(ZIP_HIST_10_90) == F)
...
# Extract subset of covariates
tecos_phys_subset_10_90 <- tecos_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_10_90,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
tecos_phys_subset_10_90_race <- tecos_phys_subset_10_90 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
tecos_phys_subset_10_90 <- left_join(tecos_phys_subset_10_90, tecos_phys_subset_10_90_race, by = "PID")

tecos_phys_subset_10_90_region <- tecos_phys_subset_10_90 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
tecos_phys_subset_10_90 <- left_join(tecos_phys_subset_10_90, tecos_phys_subset_10_90_region, by = "PID")

# Remove original IV, PID, race, and region variables
tecos_phys_subset_10_90_exposure <- tecos_phys_subset_10_90 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_10_90)

# Obtain mean value of each variable by exposure status
tecos_phys_10_90_exposure_mean <- tecos_phys_subset_10_90_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
tecos_phys_10_90_exposure_mean <- tecos_phys_10_90_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_10_90_exposure_mean <- melt(setDT(tecos_phys_10_90_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
tecos_phys_10_90_exp_mean <- tecos_phys_10_90_exposure_mean %>% filter(EXPOSURE == 1)
tecos_phys_10_90_unexp_mean <- tecos_phys_10_90_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
tecos_phys_10_90_exp_mean <- left_join(tecos_phys_10_90_exp_mean, tecos_phys_10_90_unexp_mean, by = "covariate")

# Calculate difference in means
tecos_phys_10_90_exp_mean$difference <- (tecos_phys_10_90_exp_mean$MEAN.x - tecos_phys_10_90_exp_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_10_90_exp_mean <- tecos_phys_10_90_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_10_90_exp_mean$contrast <- "By Sitagliptin"
tecos_phys_10_90_exp_mean$scale <- 1

# Reorder the variables
tecos_phys_10_90_exp_mean <- tecos_phys_10_90_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
tecos_phys_subset_10_90_iv <- tecos_phys_subset_10_90 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
tecos_phys_10_90_iv_mean <- tecos_phys_subset_10_90_iv %>% group_by(ZIP_HIST_10_90) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
tecos_phys_10_90_iv_mean <- tecos_phys_10_90_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_10_90_iv_mean <- melt(setDT(tecos_phys_10_90_iv_mean), id.vars = c("ZIP_HIST_10_90"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_10_90_iv1_mean <- tecos_phys_10_90_iv_mean %>% filter(ZIP_HIST_10_90 == 1)
tecos_phys_10_90_iv0_mean <- tecos_phys_10_90_iv_mean %>% filter(ZIP_HIST_10_90 == 0)

# Join means together
tecos_phys_10_90_iv_mean <- left_join(tecos_phys_10_90_iv1_mean, tecos_phys_10_90_iv0_mean, by = "covariate")

# Calculate difference in means
tecos_phys_10_90_iv_mean$difference <- (tecos_phys_10_90_iv_mean$MEAN.x - tecos_phys_10_90_iv_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_10_90_iv_mean <- tecos_phys_10_90_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_10_90_iv_mean$contrast <- "By Area 10-90 Candidate Instrument"
tecos_phys_10_90_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(ZIP_HIST_10_90) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
tecos_phys_10_90_iv_mean <- tecos_phys_10_90_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
tecos_phys_10_90_bias <- rbind(tecos_phys_10_90_exp_mean,tecos_phys_10_90_iv_mean)

# Bias component plot
graphfun_singleIV(tecos_phys_10_90_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_ZIP_HIST_10_90_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$ZIP_HIST_20_80_COMPLIER <- ifelse(tecos_phys$ZIP_HIST_20_80 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_zip_hist_20_80_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(ZIP_HIST_20_80) == FALSE),
                                                                 factorVars = tecos_factor_vars_ps,
                                                                 strata = "ZIP_HIST_20_80_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_zip_hist_20_80_compliers_export <- print(tecos_tableone_all_iv_zip_hist_20_80_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_zip_hist_20_80_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_zip_hist_20_80_compliers.csv")

# Distribution of outcomes by ZCTA exposure and by actual exposure
tecos_iv_zip_hist_20_80_table <- filter(tecos_phys, is.na(ZIP_HIST_20_80) == F)
...
# Extract subset of covariates
tecos_phys_subset_20_80 <- tecos_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_20_80,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
tecos_phys_subset_20_80_race <- tecos_phys_subset_20_80 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
tecos_phys_subset_20_80 <- left_join(tecos_phys_subset_20_80, tecos_phys_subset_20_80_race, by = "PID")

tecos_phys_subset_20_80_region <- tecos_phys_subset_20_80 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
tecos_phys_subset_20_80 <- left_join(tecos_phys_subset_20_80, tecos_phys_subset_20_80_region, by = "PID")

# Remove original IV, PID, race, and region variables
tecos_phys_subset_20_80_exposure <- tecos_phys_subset_20_80 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_20_80)

# Obtain mean value of each variable by exposure status
tecos_phys_20_80_exposure_mean <- tecos_phys_subset_20_80_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
tecos_phys_20_80_exposure_mean <- tecos_phys_20_80_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_20_80_exposure_mean <- melt(setDT(tecos_phys_20_80_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
tecos_phys_20_80_exp_mean <- tecos_phys_20_80_exposure_mean %>% filter(EXPOSURE == 1)
tecos_phys_20_80_unexp_mean <- tecos_phys_20_80_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
tecos_phys_20_80_exp_mean <- left_join(tecos_phys_20_80_exp_mean, tecos_phys_20_80_unexp_mean, by = "covariate")

# Calculate difference in means
tecos_phys_20_80_exp_mean$difference <- (tecos_phys_20_80_exp_mean$MEAN.x - tecos_phys_20_80_exp_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_20_80_exp_mean <- tecos_phys_20_80_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_20_80_exp_mean$contrast <- "By Sitagliptin"
tecos_phys_20_80_exp_mean$scale <- 1

# Reorder the variables
tecos_phys_20_80_exp_mean <- tecos_phys_20_80_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
tecos_phys_subset_20_80_iv <- tecos_phys_subset_20_80 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
tecos_phys_20_80_iv_mean <- tecos_phys_subset_20_80_iv %>% group_by(ZIP_HIST_20_80) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
tecos_phys_20_80_iv_mean <- tecos_phys_20_80_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_20_80_iv_mean <- melt(setDT(tecos_phys_20_80_iv_mean), id.vars = c("ZIP_HIST_20_80"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_20_80_iv1_mean <- tecos_phys_20_80_iv_mean %>% filter(ZIP_HIST_20_80 == 1)
tecos_phys_20_80_iv0_mean <- tecos_phys_20_80_iv_mean %>% filter(ZIP_HIST_20_80 == 0)

# Join means together
tecos_phys_20_80_iv_mean <- left_join(tecos_phys_20_80_iv1_mean, tecos_phys_20_80_iv0_mean, by = "covariate")

# Calculate difference in means
tecos_phys_20_80_iv_mean$difference <- (tecos_phys_20_80_iv_mean$MEAN.x - tecos_phys_20_80_iv_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_20_80_iv_mean <- tecos_phys_20_80_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_20_80_iv_mean$contrast <- "By Area 20-80 Candidate Instrument"
tecos_phys_20_80_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(ZIP_HIST_20_80) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
tecos_phys_20_80_iv_mean <- tecos_phys_20_80_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
tecos_phys_20_80_bias <- rbind(tecos_phys_20_80_exp_mean,tecos_phys_20_80_iv_mean)

# Bias component plot
graphfun_singleIV(tecos_phys_20_80_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_ZIP_HIST_20_80_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$ZIP_HIST_30_70_COMPLIER <- ifelse(tecos_phys$ZIP_HIST_30_70 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_zip_hist_30_70_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(ZIP_HIST_30_70) == FALSE),
                                                                 factorVars = tecos_factor_vars_ps,
                                                                 strata = "ZIP_HIST_30_70_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_zip_hist_30_70_compliers_export <- print(tecos_tableone_all_iv_zip_hist_30_70_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_zip_hist_30_70_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_zip_hist_30_70_compliers.csv")

# Distribution of outcomes by ZCTA exposure and by actual exposure
tecos_iv_zip_hist_30_70_table <- filter(tecos_phys, is.na(ZIP_HIST_30_70) == F)
...
# Extract subset of covariates
tecos_phys_subset_30_70 <- tecos_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_30_70,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
tecos_phys_subset_30_70_race <- tecos_phys_subset_30_70 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
tecos_phys_subset_30_70 <- left_join(tecos_phys_subset_30_70, tecos_phys_subset_30_70_race, by = "PID")

tecos_phys_subset_30_70_region <- tecos_phys_subset_30_70 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
tecos_phys_subset_30_70 <- left_join(tecos_phys_subset_30_70, tecos_phys_subset_30_70_region, by = "PID")

# Remove original IV, PID, race, and region variables
tecos_phys_subset_30_70_exposure <- tecos_phys_subset_30_70 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_30_70)

# Obtain mean value of each variable by exposure status
tecos_phys_30_70_exposure_mean <- tecos_phys_subset_30_70_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
tecos_phys_30_70_exposure_mean <- tecos_phys_30_70_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_30_70_exposure_mean <- melt(setDT(tecos_phys_30_70_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
tecos_phys_30_70_exp_mean <- tecos_phys_30_70_exposure_mean %>% filter(EXPOSURE == 1)
tecos_phys_30_70_unexp_mean <- tecos_phys_30_70_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
tecos_phys_30_70_exp_mean <- left_join(tecos_phys_30_70_exp_mean, tecos_phys_30_70_unexp_mean, by = "covariate")

# Calculate difference in means
tecos_phys_30_70_exp_mean$difference <- (tecos_phys_30_70_exp_mean$MEAN.x - tecos_phys_30_70_exp_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_30_70_exp_mean <- tecos_phys_30_70_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_30_70_exp_mean$contrast <- "By Sitagliptin"
tecos_phys_30_70_exp_mean$scale <- 1

# Reorder the variables
tecos_phys_30_70_exp_mean <- tecos_phys_30_70_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
tecos_phys_subset_30_70_iv <- tecos_phys_subset_30_70 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
tecos_phys_30_70_iv_mean <- tecos_phys_subset_30_70_iv %>% group_by(ZIP_HIST_30_70) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
tecos_phys_30_70_iv_mean <- tecos_phys_30_70_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_30_70_iv_mean <- melt(setDT(tecos_phys_30_70_iv_mean), id.vars = c("ZIP_HIST_30_70"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_30_70_iv1_mean <- tecos_phys_30_70_iv_mean %>% filter(ZIP_HIST_30_70 == 1)
tecos_phys_30_70_iv0_mean <- tecos_phys_30_70_iv_mean %>% filter(ZIP_HIST_30_70 == 0)

# Join means together
tecos_phys_30_70_iv_mean <- left_join(tecos_phys_30_70_iv1_mean, tecos_phys_30_70_iv0_mean, by = "covariate")

# Calculate difference in means
tecos_phys_30_70_iv_mean$difference <- (tecos_phys_30_70_iv_mean$MEAN.x - tecos_phys_30_70_iv_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_30_70_iv_mean <- tecos_phys_30_70_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_30_70_iv_mean$contrast <- "By Area 30-70 Candidate Instrument"
tecos_phys_30_70_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(ZIP_HIST_30_70) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
tecos_phys_30_70_iv_mean <- tecos_phys_30_70_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
tecos_phys_30_70_bias <- rbind(tecos_phys_30_70_exp_mean,tecos_phys_30_70_iv_mean)

# Bias component plot
graphfun_singleIV(tecos_phys_30_70_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_ZIP_HIST_30_70_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$ZIP_HIST_40_60_COMPLIER <- ifelse(tecos_phys$ZIP_HIST_40_60 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_zip_hist_40_60_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(ZIP_HIST_40_60) == FALSE),
                                                                 factorVars = tecos_factor_vars_ps,
                                                                 strata = "ZIP_HIST_40_60_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_zip_hist_40_60_compliers_export <- print(tecos_tableone_all_iv_zip_hist_40_60_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_zip_hist_40_60_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_zip_hist_40_60_compliers.csv")

# Distribution of outcomes by ZCTA exposure and by actual exposure
tecos_iv_zip_hist_40_60_table <- filter(tecos_phys, is.na(ZIP_HIST_40_60) == F)
...
# Extract subset of covariates
tecos_phys_subset_40_60 <- tecos_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_40_60,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
tecos_phys_subset_40_60_race <- tecos_phys_subset_40_60 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
tecos_phys_subset_40_60 <- left_join(tecos_phys_subset_40_60, tecos_phys_subset_40_60_race, by = "PID")

tecos_phys_subset_40_60_region <- tecos_phys_subset_40_60 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
tecos_phys_subset_40_60 <- left_join(tecos_phys_subset_40_60, tecos_phys_subset_40_60_region, by = "PID")

# Remove original IV, PID, race, and region variables
tecos_phys_subset_40_60_exposure <- tecos_phys_subset_40_60 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_40_60)

# Obtain mean value of each variable by exposure status
tecos_phys_40_60_exposure_mean <- tecos_phys_subset_40_60_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
tecos_phys_40_60_exposure_mean <- tecos_phys_40_60_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_40_60_exposure_mean <- melt(setDT(tecos_phys_40_60_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
tecos_phys_40_60_exp_mean <- tecos_phys_40_60_exposure_mean %>% filter(EXPOSURE == 1)
tecos_phys_40_60_unexp_mean <- tecos_phys_40_60_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
tecos_phys_40_60_exp_mean <- left_join(tecos_phys_40_60_exp_mean, tecos_phys_40_60_unexp_mean, by = "covariate")

# Calculate difference in means
tecos_phys_40_60_exp_mean$difference <- (tecos_phys_40_60_exp_mean$MEAN.x - tecos_phys_40_60_exp_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_40_60_exp_mean <- tecos_phys_40_60_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_40_60_exp_mean$contrast <- "By Sitagliptin"
tecos_phys_40_60_exp_mean$scale <- 1

# Reorder the variables
tecos_phys_40_60_exp_mean <- tecos_phys_40_60_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
tecos_phys_subset_40_60_iv <- tecos_phys_subset_40_60 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
tecos_phys_40_60_iv_mean <- tecos_phys_subset_40_60_iv %>% group_by(ZIP_HIST_40_60) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
tecos_phys_40_60_iv_mean <- tecos_phys_40_60_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_40_60_iv_mean <- melt(setDT(tecos_phys_40_60_iv_mean), id.vars = c("ZIP_HIST_40_60"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_40_60_iv1_mean <- tecos_phys_40_60_iv_mean %>% filter(ZIP_HIST_40_60 == 1)
tecos_phys_40_60_iv0_mean <- tecos_phys_40_60_iv_mean %>% filter(ZIP_HIST_40_60 == 0)

# Join means together
tecos_phys_40_60_iv_mean <- left_join(tecos_phys_40_60_iv1_mean, tecos_phys_40_60_iv0_mean, by = "covariate")

# Calculate difference in means
tecos_phys_40_60_iv_mean$difference <- (tecos_phys_40_60_iv_mean$MEAN.x - tecos_phys_40_60_iv_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_40_60_iv_mean <- tecos_phys_40_60_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_40_60_iv_mean$contrast <- "By Area 40-60 Candidate Instrument"
tecos_phys_40_60_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(ZIP_HIST_40_60) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
tecos_phys_40_60_iv_mean <- tecos_phys_40_60_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
tecos_phys_40_60_bias <- rbind(tecos_phys_40_60_exp_mean,tecos_phys_40_60_iv_mean)

# Bias component plot
graphfun_singleIV(tecos_phys_40_60_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_ZIP_HIST_40_60_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$ZIP_HIST_50_50_COMPLIER <- ifelse(tecos_phys$ZIP_HIST_50_50 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_zip_hist_50_50_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(ZIP_HIST_50_50) == FALSE),
                                                                 factorVars = tecos_factor_vars_ps,
                                                                 strata = "ZIP_HIST_50_50_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_zip_hist_50_50_compliers_export <- print(tecos_tableone_all_iv_zip_hist_50_50_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_zip_hist_50_50_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_zip_hist_50_50_compliers.csv")

# Distribution of outcomes by ZCTA exposure and by actual exposure
tecos_iv_zip_hist_50_50_table <- filter(tecos_phys, is.na(ZIP_HIST_50_50) == F)
...
# Extract subset of covariates
tecos_phys_subset_50_50 <- tecos_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_50_50,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
tecos_phys_subset_50_50_race <- tecos_phys_subset_50_50 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
tecos_phys_subset_50_50 <- left_join(tecos_phys_subset_50_50, tecos_phys_subset_50_50_race, by = "PID")

tecos_phys_subset_50_50_region <- tecos_phys_subset_50_50 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
tecos_phys_subset_50_50 <- left_join(tecos_phys_subset_50_50, tecos_phys_subset_50_50_region, by = "PID")

# Remove original IV, PID, race, and region variables
tecos_phys_subset_50_50_exposure <- tecos_phys_subset_50_50 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_50_50)

# Obtain mean value of each variable by exposure status
tecos_phys_50_50_exposure_mean <- tecos_phys_subset_50_50_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
tecos_phys_50_50_exposure_mean <- tecos_phys_50_50_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_50_50_exposure_mean <- melt(setDT(tecos_phys_50_50_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
tecos_phys_50_50_exp_mean <- tecos_phys_50_50_exposure_mean %>% filter(EXPOSURE == 1)
tecos_phys_50_50_unexp_mean <- tecos_phys_50_50_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
tecos_phys_50_50_exp_mean <- left_join(tecos_phys_50_50_exp_mean, tecos_phys_50_50_unexp_mean, by = "covariate")

# Calculate difference in means
tecos_phys_50_50_exp_mean$difference <- (tecos_phys_50_50_exp_mean$MEAN.x - tecos_phys_50_50_exp_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_50_50_exp_mean <- tecos_phys_50_50_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_50_50_exp_mean$contrast <- "By Sitagliptin"
tecos_phys_50_50_exp_mean$scale <- 1

# Reorder the variables
tecos_phys_50_50_exp_mean <- tecos_phys_50_50_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
tecos_phys_subset_50_50_iv <- tecos_phys_subset_50_50 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
tecos_phys_50_50_iv_mean <- tecos_phys_subset_50_50_iv %>% group_by(ZIP_HIST_50_50) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
tecos_phys_50_50_iv_mean <- tecos_phys_50_50_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_50_50_iv_mean <- melt(setDT(tecos_phys_50_50_iv_mean), id.vars = c("ZIP_HIST_50_50"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_50_50_iv1_mean <- tecos_phys_50_50_iv_mean %>% filter(ZIP_HIST_50_50 == 1)
tecos_phys_50_50_iv0_mean <- tecos_phys_50_50_iv_mean %>% filter(ZIP_HIST_50_50 == 0)

# Join means together
tecos_phys_50_50_iv_mean <- left_join(tecos_phys_50_50_iv1_mean, tecos_phys_50_50_iv0_mean, by = "covariate")

# Calculate difference in means
tecos_phys_50_50_iv_mean$difference <- (tecos_phys_50_50_iv_mean$MEAN.x - tecos_phys_50_50_iv_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_50_50_iv_mean <- tecos_phys_50_50_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_50_50_iv_mean$contrast <- "By Area 50-50 Candidate Instrument"
tecos_phys_50_50_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(ZIP_HIST_50_50) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
tecos_phys_50_50_iv_mean <- tecos_phys_50_50_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
tecos_phys_50_50_bias <- rbind(tecos_phys_50_50_exp_mean,tecos_phys_50_50_iv_mean)

# Bias component plot
graphfun_singleIV(tecos_phys_50_50_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_ZIP_HIST_50_50_Bias_Component_Plot")
...
zcta_shp$ZCTA_USE <- zcta_shp$ZCTA5CE10
zcta_shp_tecos <- merge(zcta_shp, tecos_exp_dist_5, by = 'ZCTA_USE')

# Convert zcta and state st files to sf
zcta_shp_tecos <- st_as_sf(zcta_shp_tecos)
state_shp <- st_as_sf(state_shp)

# Restrict to ZCTAs with data
zcta_shp_tecos <- zcta_shp_tecos %>% filter(is.na(prop_exp) == FALSE)
...
# Add complier variable
tecos_phys$PHYS_PREF_INST_COMPLIER <- ifelse(tecos_phys$PHYS_PREF_INST == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_phys_pref_inst_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(PHYS_PREF_INST) == FALSE),
                                                                 factorVars = tecos_factor_vars_ps,
                                                                 strata = "PHYS_PREF_INST_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_phys_pref_inst_compliers_export <- print(tecos_tableone_all_iv_phys_pref_inst_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_phys_pref_inst_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_phys_pref_inst_compliers.csv")
...
# Extract subset of covariates
tecos_phys_subset_inst <- tecos_phys %>% select(PID, EXPOSURE,
                                                 PHYS_PREF_INST,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
tecos_phys_subset_inst_race <- tecos_phys_subset_inst %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
tecos_phys_subset_inst <- left_join(tecos_phys_subset_inst, tecos_phys_subset_inst_race, by = "PID")

tecos_phys_subset_inst_region <- tecos_phys_subset_inst %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
tecos_phys_subset_inst <- left_join(tecos_phys_subset_inst, tecos_phys_subset_inst_region, by = "PID")

# Remove original IV, PID, race, and region variables
tecos_phys_subset_inst_exposure <- tecos_phys_subset_inst %>% select(-PID, -RACE, -REGION, -PHYS_PREF_INST)

# Obtain mean value of each variable by exposure status
tecos_phys_inst_exposure_mean <- tecos_phys_subset_inst_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
tecos_phys_inst_exposure_mean <- tecos_phys_inst_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_inst_exposure_mean <- melt(setDT(tecos_phys_inst_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
tecos_phys_inst_exp_mean <- tecos_phys_inst_exposure_mean %>% filter(EXPOSURE == 1)
tecos_phys_inst_unexp_mean <- tecos_phys_inst_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
tecos_phys_inst_exp_mean <- left_join(tecos_phys_inst_exp_mean, tecos_phys_inst_unexp_mean, by = "covariate")

# Calculate difference in means
tecos_phys_inst_exp_mean$difference <- (tecos_phys_inst_exp_mean$MEAN.x - tecos_phys_inst_exp_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_inst_exp_mean <- tecos_phys_inst_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_inst_exp_mean$contrast <- "By Sitagliptin"
tecos_phys_inst_exp_mean$scale <- 1

# Reorder the variables
tecos_phys_inst_exp_mean <- tecos_phys_inst_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
tecos_phys_subset_inst_iv <- tecos_phys_subset_inst %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
tecos_phys_inst_iv_mean <- tecos_phys_subset_inst_iv %>% group_by(PHYS_PREF_INST) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Rename the variables
tecos_phys_inst_iv_mean <- tecos_phys_inst_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
tecos_phys_inst_iv_mean <- melt(setDT(tecos_phys_inst_iv_mean), id.vars = c("PHYS_PREF_INST"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_inst_iv1_mean <- tecos_phys_inst_iv_mean %>% filter(PHYS_PREF_INST == 1)
tecos_phys_inst_iv0_mean <- tecos_phys_inst_iv_mean %>% filter(PHYS_PREF_INST == 0)

# Join means together
tecos_phys_inst_iv_mean <- left_join(tecos_phys_inst_iv1_mean, tecos_phys_inst_iv0_mean, by = "covariate")

# Calculate difference in means
tecos_phys_inst_iv_mean$difference <- (tecos_phys_inst_iv_mean$MEAN.x - tecos_phys_inst_iv_mean$MEAN.y)

# Keep only covariates and difference in means
tecos_phys_inst_iv_mean <- tecos_phys_inst_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
tecos_phys_inst_iv_mean$contrast <- "By Instantaneous Physician Preference Candidate Instrument"
tecos_phys_inst_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(PHYS_PREF_INST) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
tecos_phys_inst_iv_mean <- tecos_phys_inst_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
tecos_phys_inst_bias <- rbind(tecos_phys_inst_exp_mean,tecos_phys_inst_iv_mean)

# Bias component plot
graphfun_singleIV(tecos_phys_inst_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_PHYS_PREF_INST_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$PHYS_PREF_GLOBAL_0_100_COMPLIER <- ifelse(tecos_phys$PHYS_PREF_GLOBAL_0_100 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_phys_pref_global_0_100_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(PHYS_PREF_GLOBAL_0_100) == FALSE),
                                                                         factorVars = tecos_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_0_100_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_phys_pref_global_0_100_compliers_export <- print(tecos_tableone_all_iv_phys_pref_global_0_100_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_phys_pref_global_0_100_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_phys_pref_global_0_100_compliers.csv")
...
# Extract subset of covariates
tecos_phys_subset_0_100 <- tecos_phys %>% select(PID, EXPOSURE,
                                                 PHYS_PREF_GLOBAL_0_100,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
                                                 REGION,
...
# Remove original IV, PID, race, and region variables
tecos_phys_subset_0_100_exposure <- tecos_phys_subset_0_100 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_0_100)
...
# Obtain mean value of each variable by exposure status
tecos_phys_0_100_iv_mean <- tecos_phys_subset_0_100_iv %>% group_by(PHYS_PREF_GLOBAL_0_100) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Convert from wide to long
tecos_phys_0_100_iv_mean <- melt(setDT(tecos_phys_0_100_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_0_100"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_0_100_iv1_mean <- tecos_phys_0_100_iv_mean %>% filter(PHYS_PREF_GLOBAL_0_100 == 1)
tecos_phys_0_100_iv0_mean <- tecos_phys_0_100_iv_mean %>% filter(PHYS_PREF_GLOBAL_0_100 == 0)
...
# Add treatment indicator and scale value
tecos_phys_0_100_iv_mean$contrast <- "By Physician Preference 0-100 Candidate Instrument"
tecos_phys_0_100_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(PHYS_PREF_GLOBAL_0_100) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(tecos_phys_0_100_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_PHYS_PREF_GLOBAL_0_100_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$PHYS_PREF_GLOBAL_10_90_COMPLIER <- ifelse(tecos_phys$PHYS_PREF_GLOBAL_10_90 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_phys_pref_global_10_90_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(PHYS_PREF_GLOBAL_10_90) == FALSE),
                                                                         factorVars = tecos_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_10_90_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_phys_pref_global_10_90_compliers_export <- print(tecos_tableone_all_iv_phys_pref_global_10_90_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_phys_pref_global_10_90_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_phys_pref_global_10_90_compliers.csv")
...
# Remove original IV, PID, race, and region variables
tecos_phys_subset_10_90_exposure <- tecos_phys_subset_10_90 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_10_90)
...
# Obtain mean value of each variable by exposure status
tecos_phys_10_90_iv_mean <- tecos_phys_subset_10_90_iv %>% group_by(PHYS_PREF_GLOBAL_10_90) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
tecos_phys_10_90_iv_mean <- melt(setDT(tecos_phys_10_90_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_10_90"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_10_90_iv1_mean <- tecos_phys_10_90_iv_mean %>% filter(PHYS_PREF_GLOBAL_10_90 == 1)
tecos_phys_10_90_iv0_mean <- tecos_phys_10_90_iv_mean %>% filter(PHYS_PREF_GLOBAL_10_90 == 0)
...
# Add treatment indicator and scale value
tecos_phys_10_90_iv_mean$contrast <- "By Physician Preference 10-90 Candidate Instrument"
tecos_phys_10_90_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(PHYS_PREF_GLOBAL_10_90) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(tecos_phys_10_90_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_PHYS_PREF_GLOBAL_10_90_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$PHYS_PREF_GLOBAL_20_80_COMPLIER <- ifelse(tecos_phys$PHYS_PREF_GLOBAL_20_80 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_phys_pref_global_20_80_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(PHYS_PREF_GLOBAL_20_80) == FALSE),
                                                                         factorVars = tecos_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_20_80_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_phys_pref_global_20_80_compliers_export <- print(tecos_tableone_all_iv_phys_pref_global_20_80_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_phys_pref_global_20_80_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_phys_pref_global_20_80_compliers.csv")
...
# Remove original IV, PID, race, and region variables
tecos_phys_subset_20_80_exposure <- tecos_phys_subset_20_80 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_20_80)
...
# Obtain mean value of each variable by exposure status
tecos_phys_20_80_iv_mean <- tecos_phys_subset_20_80_iv %>% group_by(PHYS_PREF_GLOBAL_20_80) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
tecos_phys_20_80_iv_mean <- melt(setDT(tecos_phys_20_80_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_20_80"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_20_80_iv1_mean <- tecos_phys_20_80_iv_mean %>% filter(PHYS_PREF_GLOBAL_20_80 == 1)
tecos_phys_20_80_iv0_mean <- tecos_phys_20_80_iv_mean %>% filter(PHYS_PREF_GLOBAL_20_80 == 0)
...
# Add treatment indicator and scale value
tecos_phys_20_80_iv_mean$contrast <- "By Physician Preference 20-80 Candidate Instrument"
tecos_phys_20_80_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(PHYS_PREF_GLOBAL_20_80) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(tecos_phys_20_80_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_PHYS_PREF_GLOBAL_20_80_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$PHYS_PREF_GLOBAL_30_70_COMPLIER <- ifelse(tecos_phys$PHYS_PREF_GLOBAL_30_70 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_phys_pref_global_30_70_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(PHYS_PREF_GLOBAL_30_70) == FALSE),
                                                                         factorVars = tecos_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_30_70_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_phys_pref_global_30_70_compliers_export <- print(tecos_tableone_all_iv_phys_pref_global_30_70_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_phys_pref_global_30_70_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_phys_pref_global_30_70_compliers.csv")
...
# Remove original IV, PID, race, and region variables
tecos_phys_subset_30_70_exposure <- tecos_phys_subset_30_70 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_30_70)
...
# Obtain mean value of each variable by exposure status
tecos_phys_30_70_iv_mean <- tecos_phys_subset_30_70_iv %>% group_by(PHYS_PREF_GLOBAL_30_70) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
tecos_phys_30_70_iv_mean <- melt(setDT(tecos_phys_30_70_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_30_70"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_30_70_iv1_mean <- tecos_phys_30_70_iv_mean %>% filter(PHYS_PREF_GLOBAL_30_70 == 1)
tecos_phys_30_70_iv0_mean <- tecos_phys_30_70_iv_mean %>% filter(PHYS_PREF_GLOBAL_30_70 == 0)
...
# Add treatment indicator and scale value
tecos_phys_30_70_iv_mean$contrast <- "By Physician Preference 0-100 Candidate Instrument"
tecos_phys_30_70_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(PHYS_PREF_GLOBAL_30_70) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(tecos_phys_30_70_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_PHYS_PREF_GLOBAL_30_70_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$PHYS_PREF_GLOBAL_40_60_COMPLIER <- ifelse(tecos_phys$PHYS_PREF_GLOBAL_40_60 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_phys_pref_global_40_60_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(PHYS_PREF_GLOBAL_40_60) == FALSE),
                                                                         factorVars = tecos_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_40_60_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_phys_pref_global_40_60_compliers_export <- print(tecos_tableone_all_iv_phys_pref_global_40_60_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_phys_pref_global_40_60_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_phys_pref_global_40_60_compliers.csv")
...
# Remove original IV, PID, race, and region variables
tecos_phys_subset_40_60_exposure <- tecos_phys_subset_40_60 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_40_60)
...
# Obtain mean value of each variable by exposure status
tecos_phys_40_60_iv_mean <- tecos_phys_subset_40_60_iv %>% group_by(PHYS_PREF_GLOBAL_40_60) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
tecos_phys_40_60_iv_mean <- melt(setDT(tecos_phys_40_60_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_40_60"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_40_60_iv1_mean <- tecos_phys_40_60_iv_mean %>% filter(PHYS_PREF_GLOBAL_40_60 == 1)
tecos_phys_40_60_iv0_mean <- tecos_phys_40_60_iv_mean %>% filter(PHYS_PREF_GLOBAL_40_60 == 0)
...
# Add treatment indicator and scale value
tecos_phys_40_60_iv_mean$contrast <- "By Physician Preference 0-100 Candidate Instrument"
tecos_phys_40_60_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(PHYS_PREF_GLOBAL_40_60) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(tecos_phys_40_60_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_PHYS_PREF_GLOBAL_40_60_Bias_Component_Plot")
...
# Add complier variable
tecos_phys$PHYS_PREF_GLOBAL_50_50_COMPLIER <- ifelse(tecos_phys$PHYS_PREF_GLOBAL_50_50 == tecos_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
tecos_tableone_all_iv_phys_pref_global_50_50_compliers <- CreateTableOne(vars = tecos_vars_ps, data = filter(tecos_phys, is.na(PHYS_PREF_GLOBAL_50_50) == FALSE),
                                                                         factorVars = tecos_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_50_50_COMPLIER", test = FALSE, smd = TRUE)
tecos_tableone_all_iv_phys_pref_global_50_50_compliers_export <- print(tecos_tableone_all_iv_phys_pref_global_50_50_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(tecos_tableone_all_iv_phys_pref_global_50_50_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/tecos_tableone_all_iv_phys_pref_global_50_50_compliers.csv")
...
# Remove original IV, PID, race, and region variables
tecos_phys_subset_50_50_exposure <- tecos_phys_subset_50_50 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_50_50)
...
# Obtain mean value of each variable by exposure status
tecos_phys_50_50_iv_mean <- tecos_phys_subset_50_50_iv %>% group_by(PHYS_PREF_GLOBAL_50_50) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
tecos_phys_50_50_iv_mean <- melt(setDT(tecos_phys_50_50_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_50_50"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
tecos_phys_50_50_iv1_mean <- tecos_phys_50_50_iv_mean %>% filter(PHYS_PREF_GLOBAL_50_50 == 1)
tecos_phys_50_50_iv0_mean <- tecos_phys_50_50_iv_mean %>% filter(PHYS_PREF_GLOBAL_50_50 == 0)
...
# Add treatment indicator and scale value
tecos_phys_50_50_iv_mean$contrast <- "By Physician Preference 50-50 Candidate Instrument"
tecos_phys_50_50_iv_mean$scale <- 1/(tecos_phys %>% filter(is.na(PHYS_PREF_GLOBAL_50_50) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(tecos_phys_50_50_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "tecos_PHYS_PREF_GLOBAL_50_50_Bias_Component_Plot")
...
# Bias component plot graph function
graphfun_singleIV <- function(dat,location,graphname,lb=-2,ub=2) {
...
# Add complier variable
savor_phys$ZIP_HIST_0_100_COMPLIER <- ifelse(savor_phys$ZIP_HIST_0_100 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_zip_hist_0_100_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(ZIP_HIST_0_100) == FALSE),
                                                                 factorVars = savor_factor_vars_ps,
                                                                 strata = "ZIP_HIST_0_100_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_zip_hist_0_100_compliers_export <- print(savor_tableone_all_iv_zip_hist_0_100_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_zip_hist_0_100_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_zip_hist_0_100_compliers.csv")
...
# Extract subset of covariates
savor_phys_subset_0_100 <- savor_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_0_100,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
savor_phys_subset_0_100_race <- savor_phys_subset_0_100 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
savor_phys_subset_0_100 <- left_join(savor_phys_subset_0_100, savor_phys_subset_0_100_race, by = "PID")

savor_phys_subset_0_100_region <- savor_phys_subset_0_100 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
savor_phys_subset_0_100 <- left_join(savor_phys_subset_0_100, savor_phys_subset_0_100_region, by = "PID")

# Remove original IV, PID, race, and region variables
savor_phys_subset_0_100_exposure <- savor_phys_subset_0_100 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_0_100)

# Obtain mean value of each variable by exposure status
savor_phys_0_100_exposure_mean <- savor_phys_subset_0_100_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
savor_phys_0_100_exposure_mean <- savor_phys_0_100_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_0_100_exposure_mean <- melt(setDT(savor_phys_0_100_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
savor_phys_0_100_exp_mean <- savor_phys_0_100_exposure_mean %>% filter(EXPOSURE == 1)
savor_phys_0_100_unexp_mean <- savor_phys_0_100_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
savor_phys_0_100_exp_mean <- left_join(savor_phys_0_100_exp_mean, savor_phys_0_100_unexp_mean, by = "covariate")

# Calculate difference in means
savor_phys_0_100_exp_mean$difference <- (savor_phys_0_100_exp_mean$MEAN.x - savor_phys_0_100_exp_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_0_100_exp_mean <- savor_phys_0_100_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_0_100_exp_mean$contrast <- "By Saxagliptin"
savor_phys_0_100_exp_mean$scale <- 1

# Reorder the variables
savor_phys_0_100_exp_mean <- savor_phys_0_100_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
savor_phys_subset_0_100_iv <- savor_phys_subset_0_100 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
savor_phys_0_100_iv_mean <- savor_phys_subset_0_100_iv %>% group_by(ZIP_HIST_0_100) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
savor_phys_0_100_iv_mean <- savor_phys_0_100_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_0_100_iv_mean <- melt(setDT(savor_phys_0_100_iv_mean), id.vars = c("ZIP_HIST_0_100"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_0_100_iv1_mean <- savor_phys_0_100_iv_mean %>% filter(ZIP_HIST_0_100 == 1)
savor_phys_0_100_iv0_mean <- savor_phys_0_100_iv_mean %>% filter(ZIP_HIST_0_100 == 0)

# Join means together
savor_phys_0_100_iv_mean <- left_join(savor_phys_0_100_iv1_mean, savor_phys_0_100_iv0_mean, by = "covariate")

# Calculate difference in means
savor_phys_0_100_iv_mean$difference <- (savor_phys_0_100_iv_mean$MEAN.x - savor_phys_0_100_iv_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_0_100_iv_mean <- savor_phys_0_100_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_0_100_iv_mean$contrast <- "By Area 0-100 Candidate Instrument"
savor_phys_0_100_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(ZIP_HIST_0_100) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
savor_phys_0_100_iv_mean <- savor_phys_0_100_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
savor_phys_0_100_bias <- rbind(savor_phys_0_100_exp_mean,savor_phys_0_100_iv_mean)

# Bias component plot
graphfun_singleIV(savor_phys_0_100_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_ZIP_HIST_0_100_Bias_Component_Plot")
...
# Add complier variable
savor_phys$ZIP_HIST_10_90_COMPLIER <- ifelse(savor_phys$ZIP_HIST_10_90 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_zip_hist_10_90_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(ZIP_HIST_10_90) == FALSE),
                                                                 factorVars = savor_factor_vars_ps,
                                                                 strata = "ZIP_HIST_10_90_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_zip_hist_10_90_compliers_export <- print(savor_tableone_all_iv_zip_hist_10_90_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_zip_hist_10_90_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_zip_hist_10_90_compliers.csv")
...
# Extract subset of covariates
savor_phys_subset_10_90 <- savor_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_10_90,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
savor_phys_subset_10_90_race <- savor_phys_subset_10_90 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
savor_phys_subset_10_90 <- left_join(savor_phys_subset_10_90, savor_phys_subset_10_90_race, by = "PID")

savor_phys_subset_10_90_region <- savor_phys_subset_10_90 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
savor_phys_subset_10_90 <- left_join(savor_phys_subset_10_90, savor_phys_subset_10_90_region, by = "PID")

# Remove original IV, PID, race, and region variables
savor_phys_subset_10_90_exposure <- savor_phys_subset_10_90 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_10_90)

# Obtain mean value of each variable by exposure status
savor_phys_10_90_exposure_mean <- savor_phys_subset_10_90_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
savor_phys_10_90_exposure_mean <- savor_phys_10_90_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_10_90_exposure_mean <- melt(setDT(savor_phys_10_90_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
savor_phys_10_90_exp_mean <- savor_phys_10_90_exposure_mean %>% filter(EXPOSURE == 1)
savor_phys_10_90_unexp_mean <- savor_phys_10_90_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
savor_phys_10_90_exp_mean <- left_join(savor_phys_10_90_exp_mean, savor_phys_10_90_unexp_mean, by = "covariate")

# Calculate difference in means
savor_phys_10_90_exp_mean$difference <- (savor_phys_10_90_exp_mean$MEAN.x - savor_phys_10_90_exp_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_10_90_exp_mean <- savor_phys_10_90_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_10_90_exp_mean$contrast <- "By Saxagliptin"
savor_phys_10_90_exp_mean$scale <- 1

# Reorder the variables
savor_phys_10_90_exp_mean <- savor_phys_10_90_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
savor_phys_subset_10_90_iv <- savor_phys_subset_10_90 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
savor_phys_10_90_iv_mean <- savor_phys_subset_10_90_iv %>% group_by(ZIP_HIST_10_90) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
savor_phys_10_90_iv_mean <- savor_phys_10_90_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_10_90_iv_mean <- melt(setDT(savor_phys_10_90_iv_mean), id.vars = c("ZIP_HIST_10_90"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_10_90_iv1_mean <- savor_phys_10_90_iv_mean %>% filter(ZIP_HIST_10_90 == 1)
savor_phys_10_90_iv0_mean <- savor_phys_10_90_iv_mean %>% filter(ZIP_HIST_10_90 == 0)

# Join means together
savor_phys_10_90_iv_mean <- left_join(savor_phys_10_90_iv1_mean, savor_phys_10_90_iv0_mean, by = "covariate")

# Calculate difference in means
savor_phys_10_90_iv_mean$difference <- (savor_phys_10_90_iv_mean$MEAN.x - savor_phys_10_90_iv_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_10_90_iv_mean <- savor_phys_10_90_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_10_90_iv_mean$contrast <- "By Area 10-90 Candidate Instrument"
savor_phys_10_90_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(ZIP_HIST_10_90) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
savor_phys_10_90_iv_mean <- savor_phys_10_90_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
savor_phys_10_90_bias <- rbind(savor_phys_10_90_exp_mean,savor_phys_10_90_iv_mean)

# Bias component plot
graphfun_singleIV(savor_phys_10_90_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_ZIP_HIST_10_90_Bias_Component_Plot")
...
# Add complier variable
savor_phys$ZIP_HIST_20_80_COMPLIER <- ifelse(savor_phys$ZIP_HIST_20_80 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_zip_hist_20_80_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(ZIP_HIST_20_80) == FALSE),
                                                                 factorVars = savor_factor_vars_ps,
                                                                 strata = "ZIP_HIST_20_80_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_zip_hist_20_80_compliers_export <- print(savor_tableone_all_iv_zip_hist_20_80_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_zip_hist_20_80_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_zip_hist_20_80_compliers.csv")
...
# Extract subset of covariates
savor_phys_subset_20_80 <- savor_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_20_80,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
savor_phys_subset_20_80_race <- savor_phys_subset_20_80 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
savor_phys_subset_20_80 <- left_join(savor_phys_subset_20_80, savor_phys_subset_20_80_race, by = "PID")

savor_phys_subset_20_80_region <- savor_phys_subset_20_80 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
savor_phys_subset_20_80 <- left_join(savor_phys_subset_20_80, savor_phys_subset_20_80_region, by = "PID")

# Remove original IV, PID, race, and region variables
savor_phys_subset_20_80_exposure <- savor_phys_subset_20_80 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_20_80)

# Obtain mean value of each variable by exposure status
savor_phys_20_80_exposure_mean <- savor_phys_subset_20_80_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
savor_phys_20_80_exposure_mean <- savor_phys_20_80_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_20_80_exposure_mean <- melt(setDT(savor_phys_20_80_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
savor_phys_20_80_exp_mean <- savor_phys_20_80_exposure_mean %>% filter(EXPOSURE == 1)
savor_phys_20_80_unexp_mean <- savor_phys_20_80_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
savor_phys_20_80_exp_mean <- left_join(savor_phys_20_80_exp_mean, savor_phys_20_80_unexp_mean, by = "covariate")

# Calculate difference in means
savor_phys_20_80_exp_mean$difference <- (savor_phys_20_80_exp_mean$MEAN.x - savor_phys_20_80_exp_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_20_80_exp_mean <- savor_phys_20_80_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_20_80_exp_mean$contrast <- "By Saxagliptin"
savor_phys_20_80_exp_mean$scale <- 1

# Reorder the variables
savor_phys_20_80_exp_mean <- savor_phys_20_80_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
savor_phys_subset_20_80_iv <- savor_phys_subset_20_80 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
savor_phys_20_80_iv_mean <- savor_phys_subset_20_80_iv %>% group_by(ZIP_HIST_20_80) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
savor_phys_20_80_iv_mean <- savor_phys_20_80_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_20_80_iv_mean <- melt(setDT(savor_phys_20_80_iv_mean), id.vars = c("ZIP_HIST_20_80"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_20_80_iv1_mean <- savor_phys_20_80_iv_mean %>% filter(ZIP_HIST_20_80 == 1)
savor_phys_20_80_iv0_mean <- savor_phys_20_80_iv_mean %>% filter(ZIP_HIST_20_80 == 0)

# Join means together
savor_phys_20_80_iv_mean <- left_join(savor_phys_20_80_iv1_mean, savor_phys_20_80_iv0_mean, by = "covariate")

# Calculate difference in means
savor_phys_20_80_iv_mean$difference <- (savor_phys_20_80_iv_mean$MEAN.x - savor_phys_20_80_iv_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_20_80_iv_mean <- savor_phys_20_80_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_20_80_iv_mean$contrast <- "By Area 0-100 Candidate Instrument"
savor_phys_20_80_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(ZIP_HIST_20_80) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
savor_phys_20_80_iv_mean <- savor_phys_20_80_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
savor_phys_20_80_bias <- rbind(savor_phys_20_80_exp_mean,savor_phys_20_80_iv_mean)

# Bias component plot
graphfun_singleIV(savor_phys_20_80_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_ZIP_HIST_20_80_Bias_Component_Plot")
...
# Add complier variable
savor_phys$ZIP_HIST_30_70_COMPLIER <- ifelse(savor_phys$ZIP_HIST_30_70 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_zip_hist_30_70_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(ZIP_HIST_30_70) == FALSE),
                                                                 factorVars = savor_factor_vars_ps,
                                                                 strata = "ZIP_HIST_30_70_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_zip_hist_30_70_compliers_export <- print(savor_tableone_all_iv_zip_hist_30_70_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_zip_hist_30_70_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_zip_hist_30_70_compliers.csv")
...
# Extract subset of covariates
savor_phys_subset_30_70 <- savor_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_30_70,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
savor_phys_subset_30_70_race <- savor_phys_subset_30_70 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
savor_phys_subset_30_70 <- left_join(savor_phys_subset_30_70, savor_phys_subset_30_70_race, by = "PID")

savor_phys_subset_30_70_region <- savor_phys_subset_30_70 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
savor_phys_subset_30_70 <- left_join(savor_phys_subset_30_70, savor_phys_subset_30_70_region, by = "PID")

# Remove original IV, PID, race, and region variables
savor_phys_subset_30_70_exposure <- savor_phys_subset_30_70 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_30_70)

# Obtain mean value of each variable by exposure status
savor_phys_30_70_exposure_mean <- savor_phys_subset_30_70_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
savor_phys_30_70_exposure_mean <- savor_phys_30_70_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_30_70_exposure_mean <- melt(setDT(savor_phys_30_70_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
savor_phys_30_70_exp_mean <- savor_phys_30_70_exposure_mean %>% filter(EXPOSURE == 1)
savor_phys_30_70_unexp_mean <- savor_phys_30_70_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
savor_phys_30_70_exp_mean <- left_join(savor_phys_30_70_exp_mean, savor_phys_30_70_unexp_mean, by = "covariate")

# Calculate difference in means
savor_phys_30_70_exp_mean$difference <- (savor_phys_30_70_exp_mean$MEAN.x - savor_phys_30_70_exp_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_30_70_exp_mean <- savor_phys_30_70_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_30_70_exp_mean$contrast <- "By Saxagliptin"
savor_phys_30_70_exp_mean$scale <- 1

# Reorder the variables
savor_phys_30_70_exp_mean <- savor_phys_30_70_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
savor_phys_subset_30_70_iv <- savor_phys_subset_30_70 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
savor_phys_30_70_iv_mean <- savor_phys_subset_30_70_iv %>% group_by(ZIP_HIST_30_70) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
savor_phys_30_70_iv_mean <- savor_phys_30_70_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_30_70_iv_mean <- melt(setDT(savor_phys_30_70_iv_mean), id.vars = c("ZIP_HIST_30_70"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_30_70_iv1_mean <- savor_phys_30_70_iv_mean %>% filter(ZIP_HIST_30_70 == 1)
savor_phys_30_70_iv0_mean <- savor_phys_30_70_iv_mean %>% filter(ZIP_HIST_30_70 == 0)

# Join means together
savor_phys_30_70_iv_mean <- left_join(savor_phys_30_70_iv1_mean, savor_phys_30_70_iv0_mean, by = "covariate")

# Calculate difference in means
savor_phys_30_70_iv_mean$difference <- (savor_phys_30_70_iv_mean$MEAN.x - savor_phys_30_70_iv_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_30_70_iv_mean <- savor_phys_30_70_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_30_70_iv_mean$contrast <- "By Area 0-100 Candidate Instrument"
savor_phys_30_70_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(ZIP_HIST_30_70) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
savor_phys_30_70_iv_mean <- savor_phys_30_70_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
savor_phys_30_70_bias <- rbind(savor_phys_30_70_exp_mean,savor_phys_30_70_iv_mean)

# Bias component plot
graphfun_singleIV(savor_phys_30_70_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_ZIP_HIST_30_70_Bias_Component_Plot")
...
# Add complier variable
savor_phys$ZIP_HIST_40_60_COMPLIER <- ifelse(savor_phys$ZIP_HIST_40_60 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_zip_hist_40_60_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(ZIP_HIST_40_60) == FALSE),
                                                                 factorVars = savor_factor_vars_ps,
                                                                 strata = "ZIP_HIST_40_60_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_zip_hist_40_60_compliers_export <- print(savor_tableone_all_iv_zip_hist_40_60_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_zip_hist_40_60_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_zip_hist_40_60_compliers.csv")
...
# Extract subset of covariates
savor_phys_subset_40_60 <- savor_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_40_60,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
savor_phys_subset_40_60_race <- savor_phys_subset_40_60 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
savor_phys_subset_40_60 <- left_join(savor_phys_subset_40_60, savor_phys_subset_40_60_race, by = "PID")

savor_phys_subset_40_60_region <- savor_phys_subset_40_60 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
savor_phys_subset_40_60 <- left_join(savor_phys_subset_40_60, savor_phys_subset_40_60_region, by = "PID")

# Remove original IV, PID, race, and region variables
savor_phys_subset_40_60_exposure <- savor_phys_subset_40_60 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_40_60)

# Obtain mean value of each variable by exposure status
savor_phys_40_60_exposure_mean <- savor_phys_subset_40_60_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
savor_phys_40_60_exposure_mean <- savor_phys_40_60_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_40_60_exposure_mean <- melt(setDT(savor_phys_40_60_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
savor_phys_40_60_exp_mean <- savor_phys_40_60_exposure_mean %>% filter(EXPOSURE == 1)
savor_phys_40_60_unexp_mean <- savor_phys_40_60_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
savor_phys_40_60_exp_mean <- left_join(savor_phys_40_60_exp_mean, savor_phys_40_60_unexp_mean, by = "covariate")

# Calculate difference in means
savor_phys_40_60_exp_mean$difference <- (savor_phys_40_60_exp_mean$MEAN.x - savor_phys_40_60_exp_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_40_60_exp_mean <- savor_phys_40_60_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_40_60_exp_mean$contrast <- "By Saxagliptin"
savor_phys_40_60_exp_mean$scale <- 1

# Reorder the variables
savor_phys_40_60_exp_mean <- savor_phys_40_60_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
savor_phys_subset_40_60_iv <- savor_phys_subset_40_60 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
savor_phys_40_60_iv_mean <- savor_phys_subset_40_60_iv %>% group_by(ZIP_HIST_40_60) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
savor_phys_40_60_iv_mean <- savor_phys_40_60_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_40_60_iv_mean <- melt(setDT(savor_phys_40_60_iv_mean), id.vars = c("ZIP_HIST_40_60"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_40_60_iv1_mean <- savor_phys_40_60_iv_mean %>% filter(ZIP_HIST_40_60 == 1)
savor_phys_40_60_iv0_mean <- savor_phys_40_60_iv_mean %>% filter(ZIP_HIST_40_60 == 0)

# Join means together
savor_phys_40_60_iv_mean <- left_join(savor_phys_40_60_iv1_mean, savor_phys_40_60_iv0_mean, by = "covariate")

# Calculate difference in means
savor_phys_40_60_iv_mean$difference <- (savor_phys_40_60_iv_mean$MEAN.x - savor_phys_40_60_iv_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_40_60_iv_mean <- savor_phys_40_60_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_40_60_iv_mean$contrast <- "By Area 0-100 Candidate Instrument"
savor_phys_40_60_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(ZIP_HIST_40_60) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
savor_phys_40_60_iv_mean <- savor_phys_40_60_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
savor_phys_40_60_bias <- rbind(savor_phys_40_60_exp_mean,savor_phys_40_60_iv_mean)

# Bias component plot
graphfun_singleIV(savor_phys_40_60_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_ZIP_HIST_40_60_Bias_Component_Plot")
...
# Add complier variable
savor_phys$ZIP_HIST_50_50_COMPLIER <- ifelse(savor_phys$ZIP_HIST_50_50 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_zip_hist_50_50_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(ZIP_HIST_50_50) == FALSE),
                                                                 factorVars = savor_factor_vars_ps,
                                                                 strata = "ZIP_HIST_50_50_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_zip_hist_50_50_compliers_export <- print(savor_tableone_all_iv_zip_hist_50_50_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_zip_hist_50_50_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_zip_hist_50_50_compliers.csv")
...
# Extract subset of covariates
savor_phys_subset_50_50 <- savor_phys %>% select(PID, EXPOSURE,
                                                 ZIP_HIST_50_50,
                                                 AGE_CAT,
                                                 SEX,
                                                 RACE,
...
# Turn race and region into dummy variables and join back to full dataset
savor_phys_subset_50_50_race <- savor_phys_subset_50_50 %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
savor_phys_subset_50_50 <- left_join(savor_phys_subset_50_50, savor_phys_subset_50_50_race, by = "PID")

savor_phys_subset_50_50_region <- savor_phys_subset_50_50 %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
savor_phys_subset_50_50 <- left_join(savor_phys_subset_50_50, savor_phys_subset_50_50_region, by = "PID")

# Remove original IV, PID, race, and region variables
savor_phys_subset_50_50_exposure <- savor_phys_subset_50_50 %>% select(-PID, -RACE, -REGION, -ZIP_HIST_50_50)

# Obtain mean value of each variable by exposure status
savor_phys_50_50_exposure_mean <- savor_phys_subset_50_50_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                               SEX,
                                                                                                               OBESITY,
                                                                                                               OVERWEIGHT,
                                                                                                               SMOKE,
...
# Rename the variables
savor_phys_50_50_exposure_mean <- savor_phys_50_50_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                                "Male" = SEX,
                                                                                Obesity = OBESITY,
                                                                                Overweight = OVERWEIGHT,
                                                                                Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_50_50_exposure_mean <- melt(setDT(savor_phys_50_50_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
savor_phys_50_50_exp_mean <- savor_phys_50_50_exposure_mean %>% filter(EXPOSURE == 1)
savor_phys_50_50_unexp_mean <- savor_phys_50_50_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
savor_phys_50_50_exp_mean <- left_join(savor_phys_50_50_exp_mean, savor_phys_50_50_unexp_mean, by = "covariate")

# Calculate difference in means
savor_phys_50_50_exp_mean$difference <- (savor_phys_50_50_exp_mean$MEAN.x - savor_phys_50_50_exp_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_50_50_exp_mean <- savor_phys_50_50_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_50_50_exp_mean$contrast <- "By Saxagliptin"
savor_phys_50_50_exp_mean$scale <- 1

# Reorder the variables
savor_phys_50_50_exp_mean <- savor_phys_50_50_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
savor_phys_subset_50_50_iv <- savor_phys_subset_50_50 %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
savor_phys_50_50_iv_mean <- savor_phys_subset_50_50_iv %>% group_by(ZIP_HIST_50_50) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                         SEX,
                                                                                                         OBESITY,
                                                                                                         OVERWEIGHT,
                                                                                                         SMOKE,
...
# Rename the variables
savor_phys_50_50_iv_mean <- savor_phys_50_50_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                    "Male" = SEX,
                                                                    Obesity = OBESITY,
                                                                    Overweight = OVERWEIGHT,
                                                                    Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_50_50_iv_mean <- melt(setDT(savor_phys_50_50_iv_mean), id.vars = c("ZIP_HIST_50_50"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_50_50_iv1_mean <- savor_phys_50_50_iv_mean %>% filter(ZIP_HIST_50_50 == 1)
savor_phys_50_50_iv0_mean <- savor_phys_50_50_iv_mean %>% filter(ZIP_HIST_50_50 == 0)

# Join means together
savor_phys_50_50_iv_mean <- left_join(savor_phys_50_50_iv1_mean, savor_phys_50_50_iv0_mean, by = "covariate")

# Calculate difference in means
savor_phys_50_50_iv_mean$difference <- (savor_phys_50_50_iv_mean$MEAN.x - savor_phys_50_50_iv_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_50_50_iv_mean <- savor_phys_50_50_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_50_50_iv_mean$contrast <- "By Area 0-100 Candidate Instrument"
savor_phys_50_50_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(ZIP_HIST_50_50) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
savor_phys_50_50_iv_mean <- savor_phys_50_50_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
savor_phys_50_50_bias <- rbind(savor_phys_50_50_exp_mean,savor_phys_50_50_iv_mean)

# Bias component plot
graphfun_singleIV(savor_phys_50_50_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_ZIP_HIST_50_50_Bias_Component_Plot")
...
# Join frequency data to ZCTA polygons
zcta_shp$ZCTA_USE <- zcta_shp$ZCTA5CE10
zcta_shp_savor <- merge(zcta_shp, savor_exp_dist_5, by = 'ZCTA_USE')

# Convert zcta and state st files to sf
zcta_shp_savor <- st_as_sf(zcta_shp_savor)
state_shp <- st_as_sf(state_shp)

# Restrict to ZCTAs with data
zcta_shp_savor <- zcta_shp_savor %>% filter(is.na(prop_exp) == FALSE)
...
# Add complier variable
savor_phys$PHYS_PREF_INST_COMPLIER <- ifelse(savor_phys$PHYS_PREF_INST == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_phys_pref_inst_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(PHYS_PREF_INST) == FALSE),
                                                                 factorVars = savor_factor_vars_ps,
                                                                 strata = "PHYS_PREF_INST_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_phys_pref_inst_compliers_export <- print(savor_tableone_all_iv_phys_pref_inst_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_phys_pref_inst_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_phys_pref_inst_compliers.csv")
...
# Extract subset of covariates
savor_phys_subset_inst <- savor_phys %>% select(PID, EXPOSURE,
                                                PHYS_PREF_INST,
                                                AGE_CAT,
                                                SEX,
                                                RACE,
...
# Turn race and region into dummy variables and join back to full dataset
savor_phys_subset_inst_race <- savor_phys_subset_inst %>% distinct(PID, RACE) %>% 
  pivot_wider(names_from = "RACE", values_from = "RACE", names_prefix = "RACE_", values_fn = list(RACE = length), values_fill = 0)
savor_phys_subset_inst <- left_join(savor_phys_subset_inst, savor_phys_subset_inst_race, by = "PID")

savor_phys_subset_inst_region <- savor_phys_subset_inst %>% distinct(PID, REGION) %>% 
  pivot_wider(names_from = "REGION", values_from = "REGION", names_prefix = "REGION_", values_fn = list(REGION = length), values_fill = 0)
savor_phys_subset_inst <- left_join(savor_phys_subset_inst, savor_phys_subset_inst_region, by = "PID")

# Remove original IV, PID, race, and region variables
savor_phys_subset_inst_exposure <- savor_phys_subset_inst %>% select(-PID, -RACE, -REGION, -PHYS_PREF_INST)

# Obtain mean value of each variable by exposure status
savor_phys_inst_exposure_mean <- savor_phys_subset_inst_exposure %>% group_by(EXPOSURE) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                             SEX,
                                                                                                             OBESITY,
                                                                                                             OVERWEIGHT,
                                                                                                             SMOKE,
...
# Rename the variables
savor_phys_inst_exposure_mean <- savor_phys_inst_exposure_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                              "Male" = SEX,
                                                                              Obesity = OBESITY,
                                                                              Overweight = OVERWEIGHT,
                                                                              Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_inst_exposure_mean <- melt(setDT(savor_phys_inst_exposure_mean), id.vars = c("EXPOSURE"), variable.name = "covariate", value.name = "MEAN")

# Filter means from exposed and unexposed
savor_phys_inst_exp_mean <- savor_phys_inst_exposure_mean %>% filter(EXPOSURE == 1)
savor_phys_inst_unexp_mean <- savor_phys_inst_exposure_mean %>% filter(EXPOSURE == 0)

# Join means together
savor_phys_inst_exp_mean <- left_join(savor_phys_inst_exp_mean, savor_phys_inst_unexp_mean, by = "covariate")

# Calculate difference in means
savor_phys_inst_exp_mean$difference <- (savor_phys_inst_exp_mean$MEAN.x - savor_phys_inst_exp_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_inst_exp_mean <- savor_phys_inst_exp_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_inst_exp_mean$contrast <- "By Saxagliptin"
savor_phys_inst_exp_mean$scale <- 1

# Reorder the variables
savor_phys_inst_exp_mean <- savor_phys_inst_exp_mean[,c(1,3,4,2)]

# Remove original exposure, PID, race, and region variables
savor_phys_subset_inst_iv <- savor_phys_subset_inst %>% select(-PID, -RACE, -REGION, -EXPOSURE)

# Obtain mean value of each variable by exposure status
savor_phys_inst_iv_mean <- savor_phys_subset_inst_iv %>% group_by(PHYS_PREF_INST) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                       SEX,
                                                                                                       OBESITY,
                                                                                                       OVERWEIGHT,
                                                                                                       SMOKE,
...
# Rename the variables
savor_phys_inst_iv_mean <- savor_phys_inst_iv_mean %>% rename("Age ≥75" = AGE_CAT,
                                                                  "Male" = SEX,
                                                                  Obesity = OBESITY,
                                                                  Overweight = OVERWEIGHT,
                                                                  Smoking = SMOKE,
...
# Convert from wide to long
savor_phys_inst_iv_mean <- melt(setDT(savor_phys_inst_iv_mean), id.vars = c("PHYS_PREF_INST"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_inst_iv1_mean <- savor_phys_inst_iv_mean %>% filter(PHYS_PREF_INST == 1)
savor_phys_inst_iv0_mean <- savor_phys_inst_iv_mean %>% filter(PHYS_PREF_INST == 0)

# Join means together
savor_phys_inst_iv_mean <- left_join(savor_phys_inst_iv1_mean, savor_phys_inst_iv0_mean, by = "covariate")

# Calculate difference in means
savor_phys_inst_iv_mean$difference <- (savor_phys_inst_iv_mean$MEAN.x - savor_phys_inst_iv_mean$MEAN.y)

# Keep only covariates and difference in means
savor_phys_inst_iv_mean <- savor_phys_inst_iv_mean %>% select(covariate, difference)

# Add treatment indicator and scale value
savor_phys_inst_iv_mean$contrast <- "By Instantaneous Physician Preference Candidate Instrument"
savor_phys_inst_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(PHYS_PREF_INST) == F) %>% dplyr::summarize(mean(EXPOSURE)))

# Reorder the variables
savor_phys_inst_iv_mean <- savor_phys_inst_iv_mean[,c(1,3,4,2)]

# Append means from exposure and instrument
savor_phys_inst_bias <- rbind(savor_phys_inst_exp_mean,savor_phys_inst_iv_mean)

# Bias component plot
graphfun_singleIV(savor_phys_inst_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_PHYS_PREF_INST_Bias_Component_Plot")
...
# Add complier variable
savor_phys$PHYS_PREF_GLOBAL_0_100_COMPLIER <- ifelse(savor_phys$PHYS_PREF_GLOBAL_0_100 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_phys_pref_global_0_100_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(PHYS_PREF_GLOBAL_0_100) == FALSE),
                                                                 factorVars = savor_factor_vars_ps,
                                                                 strata = "PHYS_PREF_GLOBAL_0_100_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_phys_pref_global_0_100_compliers_export <- print(savor_tableone_all_iv_phys_pref_global_0_100_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_phys_pref_global_0_100_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_phys_pref_global_0_100_compliers.csv")
...
# Remove original IV, PID, race, and region variables
savor_phys_subset_0_100_exposure <- savor_phys_subset_0_100 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_0_100)
...
# Add treatment indicator and scale value
savor_phys_0_100_exp_mean$contrast <- "By Sitagliptin"
savor_phys_0_100_exp_mean$scale <- 1
...
# Obtain mean value of each variable by exposure status
savor_phys_0_100_iv_mean <- savor_phys_subset_0_100_iv %>% group_by(PHYS_PREF_GLOBAL_0_100) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
savor_phys_0_100_iv_mean <- melt(setDT(savor_phys_0_100_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_0_100"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_0_100_iv1_mean <- savor_phys_0_100_iv_mean %>% filter(PHYS_PREF_GLOBAL_0_100 == 1)
savor_phys_0_100_iv0_mean <- savor_phys_0_100_iv_mean %>% filter(PHYS_PREF_GLOBAL_0_100 == 0)
...
# Add treatment indicator and scale value
savor_phys_0_100_iv_mean$contrast <- "By Physician Preference 0-100 Candidate Instrument"
savor_phys_0_100_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(PHYS_PREF_GLOBAL_0_100) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(savor_phys_0_100_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_PHYS_PREF_GLOBAL_0_100_Bias_Component_Plot")
...
# Add complier variable
savor_phys$PHYS_PREF_GLOBAL_10_90_COMPLIER <- ifelse(savor_phys$PHYS_PREF_GLOBAL_10_90 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_phys_pref_global_10_90_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(PHYS_PREF_GLOBAL_10_90) == FALSE),
                                                                         factorVars = savor_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_10_90_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_phys_pref_global_10_90_compliers_export <- print(savor_tableone_all_iv_phys_pref_global_10_90_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_phys_pref_global_10_90_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_phys_pref_global_10_90_compliers.csv")
...
# Remove original IV, PID, race, and region variables
savor_phys_subset_10_90_exposure <- savor_phys_subset_10_90 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_10_90)
...
# Add treatment indicator and scale value
savor_phys_10_90_exp_mean$contrast <- "By Sitagliptin"
savor_phys_10_90_exp_mean$scale <- 1
...
# Obtain mean value of each variable by exposure status
savor_phys_10_90_iv_mean <- savor_phys_subset_10_90_iv %>% group_by(PHYS_PREF_GLOBAL_10_90) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
savor_phys_10_90_iv_mean <- melt(setDT(savor_phys_10_90_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_10_90"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_10_90_iv1_mean <- savor_phys_10_90_iv_mean %>% filter(PHYS_PREF_GLOBAL_10_90 == 1)
savor_phys_10_90_iv0_mean <- savor_phys_10_90_iv_mean %>% filter(PHYS_PREF_GLOBAL_10_90 == 0)
...
# Add treatment indicator and scale value
savor_phys_10_90_iv_mean$contrast <- "By Physician Preference 10-90 Candidate Instrument"
savor_phys_10_90_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(PHYS_PREF_GLOBAL_10_90) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(savor_phys_10_90_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_PHYS_PREF_GLOBAL_10_90_Bias_Component_Plot")
...
# Add complier variable
savor_phys$PHYS_PREF_GLOBAL_20_80_COMPLIER <- ifelse(savor_phys$PHYS_PREF_GLOBAL_20_80 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_phys_pref_global_20_80_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(PHYS_PREF_GLOBAL_20_80) == FALSE),
                                                                         factorVars = savor_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_20_80_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_phys_pref_global_20_80_compliers_export <- print(savor_tableone_all_iv_phys_pref_global_20_80_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_phys_pref_global_20_80_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_phys_pref_global_20_80_compliers.csv")
...
# Remove original IV, PID, race, and region variables
savor_phys_subset_20_80_exposure <- savor_phys_subset_20_80 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_20_80)
...
# Add treatment indicator and scale value
savor_phys_20_80_exp_mean$contrast <- "By Sitagliptin"
savor_phys_20_80_exp_mean$scale <- 1
...
# Obtain mean value of each variable by exposure status
savor_phys_20_80_iv_mean <- savor_phys_subset_20_80_iv %>% group_by(PHYS_PREF_GLOBAL_20_80) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
savor_phys_20_80_iv_mean <- melt(setDT(savor_phys_20_80_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_20_80"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_20_80_iv1_mean <- savor_phys_20_80_iv_mean %>% filter(PHYS_PREF_GLOBAL_20_80 == 1)
savor_phys_20_80_iv0_mean <- savor_phys_20_80_iv_mean %>% filter(PHYS_PREF_GLOBAL_20_80 == 0)
...
# Add treatment indicator and scale value
savor_phys_20_80_iv_mean$contrast <- "By Physician Preference 20-80 Candidate Instrument"
savor_phys_20_80_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(PHYS_PREF_GLOBAL_20_80) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(savor_phys_20_80_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_PHYS_PREF_GLOBAL_20_80_Bias_Component_Plot")
...
# Add complier variable
savor_phys$PHYS_PREF_GLOBAL_30_70_COMPLIER <- ifelse(savor_phys$PHYS_PREF_GLOBAL_30_70 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_phys_pref_global_30_70_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(PHYS_PREF_GLOBAL_30_70) == FALSE),
                                                                         factorVars = savor_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_30_70_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_phys_pref_global_30_70_compliers_export <- print(savor_tableone_all_iv_phys_pref_global_30_70_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_phys_pref_global_30_70_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_phys_pref_global_30_70_compliers.csv")
...
# Remove original IV, PID, race, and region variables
savor_phys_subset_30_70_exposure <- savor_phys_subset_30_70 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_30_70)
...
# Add treatment indicator and scale value
savor_phys_30_70_exp_mean$contrast <- "By Sitagliptin"
savor_phys_30_70_exp_mean$scale <- 1
...
# Obtain mean value of each variable by exposure status
savor_phys_30_70_iv_mean <- savor_phys_subset_30_70_iv %>% group_by(PHYS_PREF_GLOBAL_30_70) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
savor_phys_30_70_iv_mean <- melt(setDT(savor_phys_30_70_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_30_70"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_30_70_iv1_mean <- savor_phys_30_70_iv_mean %>% filter(PHYS_PREF_GLOBAL_30_70 == 1)
savor_phys_30_70_iv0_mean <- savor_phys_30_70_iv_mean %>% filter(PHYS_PREF_GLOBAL_30_70 == 0)
...
# Add treatment indicator and scale value
savor_phys_30_70_iv_mean$contrast <- "By Physician Preference 30-70 Candidate Instrument"
savor_phys_30_70_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(PHYS_PREF_GLOBAL_30_70) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(savor_phys_30_70_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_PHYS_PREF_GLOBAL_30_70_Bias_Component_Plot")
...
# Add complier variable
savor_phys$PHYS_PREF_GLOBAL_40_60_COMPLIER <- ifelse(savor_phys$PHYS_PREF_GLOBAL_40_60 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_phys_pref_global_40_60_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(PHYS_PREF_GLOBAL_40_60) == FALSE),
                                                                         factorVars = savor_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_40_60_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_phys_pref_global_40_60_compliers_export <- print(savor_tableone_all_iv_phys_pref_global_40_60_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_phys_pref_global_40_60_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_phys_pref_global_40_60_compliers.csv")
...
# Remove original IV, PID, race, and region variables
savor_phys_subset_40_60_exposure <- savor_phys_subset_40_60 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_40_60)
...
# Add treatment indicator and scale value
savor_phys_40_60_exp_mean$contrast <- "By Sitagliptin"
savor_phys_40_60_exp_mean$scale <- 1
...
# Obtain mean value of each variable by exposure status
savor_phys_40_60_iv_mean <- savor_phys_subset_40_60_iv %>% group_by(PHYS_PREF_GLOBAL_40_60) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
savor_phys_40_60_iv_mean <- melt(setDT(savor_phys_40_60_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_40_60"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_40_60_iv1_mean <- savor_phys_40_60_iv_mean %>% filter(PHYS_PREF_GLOBAL_40_60 == 1)
savor_phys_40_60_iv0_mean <- savor_phys_40_60_iv_mean %>% filter(PHYS_PREF_GLOBAL_40_60 == 0)
...
# Add treatment indicator and scale value
savor_phys_40_60_iv_mean$contrast <- "By Physician Preference 40-60 Candidate Instrument"
savor_phys_40_60_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(PHYS_PREF_GLOBAL_40_60) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(savor_phys_40_60_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_PHYS_PREF_GLOBAL_40_60_Bias_Component_Plot")
...
# Add complier variable
savor_phys$PHYS_PREF_GLOBAL_50_50_COMPLIER <- ifelse(savor_phys$PHYS_PREF_GLOBAL_50_50 == savor_phys$EXPOSURE,1,0)

# Table 1 at the individual level comparing compliers to full LATE population
savor_tableone_all_iv_phys_pref_global_50_50_compliers <- CreateTableOne(vars = savor_vars_ps, data = filter(savor_phys, is.na(PHYS_PREF_GLOBAL_50_50) == FALSE),
                                                                         factorVars = savor_factor_vars_ps,
                                                                         strata = "PHYS_PREF_GLOBAL_50_50_COMPLIER", test = FALSE, smd = TRUE)
savor_tableone_all_iv_phys_pref_global_50_50_compliers_export <- print(savor_tableone_all_iv_phys_pref_global_50_50_compliers, smd = TRUE, quote = TRUE, noSpaces = TRUE)
write.csv(savor_tableone_all_iv_phys_pref_global_50_50_compliers_export, file = "/Volumes/PACS$/Cordes, Jack/TableOne_Compliers/savor_tableone_all_iv_phys_pref_global_50_50_compliers.csv")
...
# Remove original IV, PID, race, and region variables
savor_phys_subset_50_50_exposure <- savor_phys_subset_50_50 %>% select(-PID, -RACE, -REGION, -PHYS_PREF_GLOBAL_50_50)
...
# Add treatment indicator and scale value
savor_phys_50_50_exp_mean$contrast <- "By Sitagliptin"
savor_phys_50_50_exp_mean$scale <- 1
...
# Obtain mean value of each variable by exposure status
savor_phys_50_50_iv_mean <- savor_phys_subset_50_50_iv %>% group_by(PHYS_PREF_GLOBAL_50_50) %>% dplyr::summarise(across(c(AGE_CAT,
                                                                                                                 SEX,
                                                                                                                 OBESITY,
                                                                                                                 OVERWEIGHT,
                                                                                                                 SMOKE,
...
# Convert from wide to long
savor_phys_50_50_iv_mean <- melt(setDT(savor_phys_50_50_iv_mean), id.vars = c("PHYS_PREF_GLOBAL_50_50"), variable.name = "covariate", value.name = "MEAN")

# Filter means from iv = 1 and iv = 0
savor_phys_50_50_iv1_mean <- savor_phys_50_50_iv_mean %>% filter(PHYS_PREF_GLOBAL_50_50 == 1)
savor_phys_50_50_iv0_mean <- savor_phys_50_50_iv_mean %>% filter(PHYS_PREF_GLOBAL_50_50 == 0)
...
# Add treatment indicator and scale value
savor_phys_50_50_iv_mean$contrast <- "By Physician Preference 50-50 Candidate Instrument"
savor_phys_50_50_iv_mean$scale <- 1/(savor_phys %>% filter(is.na(PHYS_PREF_GLOBAL_50_50) == F) %>% dplyr::summarize(mean(EXPOSURE)))
...
# Bias component plot
graphfun_singleIV(savor_phys_50_50_bias, location = "/Volumes/PACS$/Cordes, Jack/Bias_Component_Plots/", "savor_PHYS_PREF_GLOBAL_50_50_Bias_Component_Plot")
